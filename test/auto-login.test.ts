...
test('extractCredentialsFromCookieJar rejects same-named cookies outside the target host', () => {
  const parsed = extractCredentialsFromCookieJar(
    [
      { name: 'auth_token', value: 'attacker-token', domain: 'attacker.example' },
...
test('resolveBrowserSessionStatePath uses APPDATA on windows', () => {
  ...
  assert.equal(path, 'C:\\Users\\mark\\AppData\\Roaming/ontrack-cli/browser-state.json');
});
...
test('resolveSystemBrowserUserDataDirs returns platform defaults on mac', () => {
  ...
  assert.equal(dirs.some((item) => item.userDataDir.includes('/Users/demo/Library/Application Support/Google/Chrome')), true);
});

test('system browser profile reuse is disabled unless explicitly opted in', () => {
  assert.equal(isSystemBrowserProfileReuseEnabled({} as NodeJS.ProcessEnv), false);
  assert.equal(
    isSystemBrowserProfileReuseEnabled({ ONTRACK_ENABLE_SYSTEM_BROWSER_PROFILE: '1' } as NodeJS.ProcessEnv),
    true,
  );
});
...
test('saveBrowserSessionState makes its directory 0700 and state file 0600', async () => {
  ...
  try {
    await saveBrowserSessionState(
      {
        storageState: async () => ({
          cookies: [
            {
              name: 'auth_token', value: 'ontrack-token', domain: 'ontrack.infotech.monash.edu',
              path: '/', expires: -1, httpOnly: false, secure: true, sameSite: 'Lax',
            },
            {
              name: 'idp_session', value: 'must-not-persist', domain: 'monash.okta.com',
          ...
          origins: [
            {
              ...
              localStorage: [{ name: 'doubtfire_credentials_token', value: 'ontrack-token' }],
            },
            {
    ...
    const persisted = JSON.parse(await readFile(storagePath, 'utf8')) as {
      ...
      origins: Array<{ origin: string; localStorage: Array<{ value: string }> }>;
    };
...
test('buildContextOptionsWithStoredSession migrates legacy state and excludes cross-site data', async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'ontrack-legacy-browser-state-'));
  ...
  try {
    await writeFile(storagePath, JSON.stringify({
      cookies: [
        onTrackCookie,
        {
          name: 'idp_session', value: 'must-not-load', domain: 'monash.okta.com',
          path: '/', expires: -1, httpOnly: true, secure: true, sameSite: 'Lax',
        },
      ],
      origins: [
...
test('buildContextOptionsWithStoredSession rejects invalid persisted JSON', async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'ontrack-invalid-browser-state-'));
  const storagePath = join(tempRoot, 'browser-state.json');
...
test('expandSystemBrowserProfileCandidates discovers Default and Profile N folders', () => {
  const expanded = expandSystemBrowserProfileCandidates(
    ...
    ],
    {
      ...
      listDirNames: () => ['System Profile', 'Profile 2', 'Default', 'Profile 1'],
    },
  );
...
test('expandSystemBrowserProfileCandidates honors explicit profile override', () => {
  const expanded = expandSystemBrowserProfileCandidates(
    ...
    ],
    {
      ...
      pathExists: (path) => path === '/profiles/chrome' || path === '/profiles/chrome/Profile 3',
