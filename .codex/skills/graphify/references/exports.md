...
Read the password from the environment so it never appears in argv or shell
history:
...
Set `NEO4J_PASSWORD` through the user's secret manager or secure shell session
before running the command. Never print it or place it on the command line.
