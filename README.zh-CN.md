...
- Node.js `22+`
- macOS / Linux / Windows
- 建议有一个 Chromium 系浏览器用于 `ontrack login --auto`
...
如果你已经在浏览器完成登录，但更希望手动控制:

```bash
ontrack login
```

CLI 会要求你粘贴最终的 redirect URL，格式通常类似:
...
| `ontrack login --auto` | 自动 SSO 登录 | 推荐默认登录入口 |
| `ontrack login` | 手动粘贴 redirect URL 登录 | 自动登录不方便时使用 |
