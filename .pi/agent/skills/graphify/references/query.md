...
Do not save the question or answer by default. Only when the user explicitly
opts into project-local query memory, redact secrets and personal data, then
...
Replace `ORIGINAL_QUESTION` and `ANSWER` with redacted summaries, never verbatim
private content. `NODE1 NODE2` is the list of cited node labels.
...
Only after explicit opt-in, save a redacted path summary:
...
Only after explicit opt-in, save a redacted explanation:
