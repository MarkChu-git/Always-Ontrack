...
```bash
# This project pins Graphify. Installation is an explicit setup action:
#   bun run graphify:setup
EXPECTED_GRAPHIFY_VERSION="0.9.31"
PYTHON=""
GRAPHIFY_BIN=$(which graphify 2>/dev/null)
# Read the interpreter shebang from the explicitly provisioned Graphify binary.
...
# Fall back only for an already installed system package; never install here.
if [ -z "$PYTHON" ]; then PYTHON="python3"; fi
if ! "$PYTHON" -c "import graphify" 2>/dev/null; then
    echo "Graphify is not installed. Run: bun run graphify:setup" >&2
    exit 1
fi
_GRAPHIFY_VERSION=$("$PYTHON" -c "from importlib.metadata import version; print(version('graphifyy'))" 2>/dev/null)
if [ "$_GRAPHIFY_VERSION" != "$EXPECTED_GRAPHIFY_VERSION" ]; then
    echo "Graphify version mismatch: expected ${EXPECTED_GRAPHIFY_VERSION}, found ${_GRAPHIFY_VERSION:-unknown}." >&2
    echo "Run: bun run graphify:setup" >&2
    exit 1
fi
...
If the import and pinned-version check succeed, print nothing and move straight to Step 2.
...
**Project privacy override:** never auto-select a remote semantic backend merely
because an API key is present. Use Gemini only when the user explicitly selects
Gemini and approves the document, paper, and image files in scope. Otherwise
use the host-agent extraction flow below. Code extraction always remains local.
Redact secrets and personal data before sending approved content to any remote
backend.

> **No API key is read without explicit user opt-in.** By default semantic
> extraction falls to the host agent itself. On a host that dispatches
> subagents, dispatch them as written in Part B. On a host that runs the CLI
> directly and cannot dispatch subagents, do not stall: a code-only corpus has
> no semantic work, so write the empty semantic file and continue; for a corpus
> with docs, papers, or images, extract those inline or ask for approval to use a
> named backend, but never infer consent from an environment variable.
